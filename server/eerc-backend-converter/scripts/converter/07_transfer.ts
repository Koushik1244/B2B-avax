...
const main = async () => {
    ...
    const receiverAddress = process.env.TRANSFER_RECEIVER || (await wallet2.getAddress());
    const transferAmountStr = process.env.TRANSFER_AMOUNT || "40";
...
Registering user with
 Address:${senderAddress.toLowerCase()}`;
        ...
        console.log("📝 Transfer transaction sent:", transferTx.hash);
         ...
         console.log(`TRANSFER_RESULT_JSON:${JSON.stringify({ txHash: transferTx.hash, blockNumber: receipt?.blockNumber, from: senderAddress, to: receiverAddress, amount: transferAmount, tokenId: tokenId.toString() })}`);
        ...
        console.error(error);
        console.log(`TRANSFER_RESULT_JSON:${JSON.stringify({ success: false, error: error instanceof Error ? error.message : String(error) })}`);
